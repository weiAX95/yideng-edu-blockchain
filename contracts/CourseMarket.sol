...
contract CourseMarket is Ownable {
  ...
  // 事件
  event CoursePurchased(address indexed buyer, uint256 indexed courseId, string web2CourseId);
  event CourseCompleted(address indexed student, uint256 indexed courseId, uint256 certificateId);
  event CourseAdded(uint256 indexed courseId, string web2CourseId, string name);
  ...
  function purchaseCourse(string memory web2CourseId) external {
    ...
    // 转移代币
    require(yiDengToken.transferFrom(msg.sender, course.creator, course.price), 'Transfer failed');
   ...
   * @param web2CourseId Web2平台的课程ID
   */
  function verifyCourseCompletion(address student, string memory web2CourseId) external onlyOwner {
    uint256 courseId = web2ToCourseId[web2CourseId];
    require(courseId > 0, 'Course does not exist');
    ...
    string memory metadataURI = generateCertificateURI(student, web2CourseId);
    uint256 tokenId = certificate.mintCertificate(student, web2CourseId, metadataURI);
   ...
   * @param web2CourseId Web2平台的课程ID
   */
  function hasCourse(address user, string memory web2CourseId) external view returns (bool) {
    uint256 courseId = web2ToCourseId[web2CourseId];
    require(courseId > 0, 'Course does not exist');
